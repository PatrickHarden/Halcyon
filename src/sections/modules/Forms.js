...
export default class GravityForm extends React.Component{
    constructor(props) {
        super(props);
    ...
    componentDidMount() {
        ...
        console.log(this.props.section.form.id)
        // assign top-level component scope
        let component = this;
        if (this.props.section.form.id) {
            $.ajax({
                url: PropertyOptions,
                type: 'GET',
                headers: {
                ...
                success: function (data) {
                    ...
                    let signature = component.gformAuth(component.props.section.form.id, data.acf.public_api_key, data.acf.private_api_key, "GET");
                    let gformURL = SiteURL + '/gravityformsapi/forms/' + component.props.section.form.id + '?api_key=' + data.acf.public_api_key + '&signature=' + signature;
                    $.ajax({
                        url: gformURL,
                        type: 'GET',
                        headers: {
    ...
    handleSubmit(event) {
        ...
        let signature = this.gformAuth(this.props.section.form.id, this.state.publicKey, this.state.privateKey, "POST");
        let gformURL = SiteURL + '/gravityformsapi/forms/' + this.props.section.form.id + '/submissions?api_key=' + this.state.publicKey + '&signature=' + signature;
                ...
                success: function (data) {
                    if (data.status <= 202) {
                        if(data.response.is_valid){
                            $('#gform-' + component.props.section.form.id + ' input[type="submit"]').addClass('hidden');
                            $('#gform-' + component.props.section.form.id + ' .confirmation .message').html(data.response.confirmation_message);
                            $('#gform-' + component.props.section.form.id + ' .confirmation').removeClass('hidden');
                            $('#gform-' + component.props.section.form.id + ' .fields').fadeOut();
            ...
            return (
                <Row className="gform">
                    <Col xs={12}>
                        ...
                        <form title={this.state.gformTitle} className="gform" id={'gform-' + this.props.section.form.id} onSubmit={this.handleSubmit}>
